...
class RegressionModel(nn.Module):
    def __init__(self, use_cuda=False):
        ...
        self.fc1 = MNFLinear(1, 100, 16, 0, 2, 2, use_cuda=use_cuda)
        self.fc2 = MNFLinear(100, 1, 16, 0, 2, 2, use_cuda=use_cuda)

    def forward(self, x, same_noise=False):
        x = self.fc1.forward(x, same_noise=same_noise)
        x = F.relu(x)
        x = self.fc2.forward(x, same_noise=same_noise)
        return x
...
if __name__ == '__main__':
    ...
    if BAYES:
        regressor = RegressionModel(use_cuda=use_cuda)
    else:
        regressor = MLP()
    ...
    for epoch in range(1000):
        ...
        if BAYES:
            regressor.reset_noise()
            y_pred = regressor(x, same_noise=False)
            kldiv = regressor.kldiv()
            kl_reg = kldiv / 20.0
    ...
    for _ in range(20):
        if BAYES:
            regressor.reset_noise()
            y_pred = regressor(X_TEST, same_noise=True)
    ...
    plt.plot(X, Y, 'x')
    plt.ylim(-100, 100)
    plt.savefig('local_mnf_toy_regression.png')
    # plt.show()
