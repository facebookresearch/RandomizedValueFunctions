...
from qlearn.toys.model import BayesBackpropDQN


class BayesBackpropAgent():
    def __init__(self, args, env):
        ...
        self.online_net = BayesBackpropDQN(args, self.action_space)
        if args.model and os.path.isfile(args.model):
            self.online_net.load_state_dict(torch.load(args.model))
        self.online_net.train()

        self.target_net = BayesBackpropDQN(args, self.action_space)
    ...
    def learn(self, states, actions, rewards, next_states, terminals):
        ...
        # import pdb
        # pdb.set_trace()
        mse = F.smooth_l1_loss(state_action_values, target_state_action_values.detach())
        kl_reg = self.online_net.get_reg()
        loss = mse + kl_reg / self.batch_size
        # Optimize the model
