...
from qlearn.atari.bayes_backprop_agent import AtariBayesBackpropAgent
...
for seed in SEEDS:
    for game in GAMES:
        for alpha in ALPHAS:
            RUN_ID.append((seed, game, alpha))


def parse_args():
    parser = argparse.ArgumentParser("Bayes backprop DQN experiments for Atari games")
    # Environment
    ...
    parser.add_argument("--agent", type=str, default="BayesBackpropDQN", help="which agent to run")
    parser.add_argument("--discount", type=float, default=0.99, help="discount factor")
    # approximate posterior
    parser.add_argument('--hidden_dim', type=int, default=50, help='number of hidden unit used in normalizing flows')
    parser.add_argument('--n-hidden', type=int, default=0, help='number of hidden layer used in normalizing flows')
    parser.add_argument('--n-flows-q', type=int, default=1, help='number of normalizing flows using for the approximate posterior q')
    parser.add_argument('--n-flows-r', type=int, default=1, help='number of normalizing flows using for auxiliary posterior r')
    parser.add_argument('--alpha', type=float, default=0.001, help='trade-off parameter betweem KL and likelihood term')
...
if __name__ == '__main__':
    ...
    if args.run_index is not None:
        args.seed, args.env, args.alpha = RUN_ID[args.run_index]
    ...
    log_dir = '{}/{}-{}-seed-{}-alpha-{}-{}'.format(args.log_dir, args.env, args.agent, args.seed, args.alpha, date)
    save_dir = '{}/{}-{}-seed-{}-alpha-{}-{}'.format(args.save_dir, args.env, args.agent, args.seed, args.alpha, date)
    ...
    agent = AtariBayesBackpropAgent(args, env.observation_space.shape[-1], num_actions)

    # exploration = LinearSchedule(args.final_exploration_frame, args.final_exploration, 1)
    ...
    while True:
        ...
        if num_iters > args.learning_starts and num_iters % args.learning_freq == 0:
            ...
            # TODO
            td_errors, kl_reg, loss = agent.learn(obses_t, actions, rewards, obses_tp1, dones)
            # loss = agent.learn(obses_t, actions, rewards, obses_tp1, dones)
            td_errors_list.append(loss.item())
            log.add_scalar('td_errors', td_errors.item(), num_iters)
            log.add_scalar('kl_reg', kl_reg.item(), num_iters)
            log.add_scalar('loss', loss.item(), num_iters)
