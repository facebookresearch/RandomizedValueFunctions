...
class AtariMNFDQN(nn.Module):
    def __init__(self, args, input_dim, num_actions):
        ...
        self.fc1 = MNFLinear(3136, 512, args.hidden_dim, args.n_hidden, args.n_flows_q, args.n_flows_r)
        self.fc2 = MNFLinear(512, num_actions, args.hidden_dim, args.n_hidden, args.n_flows_q, args.n_flows_r)
...
class AtariMNFAgent(object):
    ...
    def learn(self, states, actions, rewards, next_states, terminals):
        ...
        state_values, kldiv = self.online_net(states, kl=True)
        state_action_values = state_values.gather(1, actions.view(-1, 1))

        if self.double_q:
            next_actions = self.online_net(next_states, kl=False).max(1)[1]
            next_state_values = self.target_net(next_states, kl=False).gather(1, next_actions.view(-1, 1))
        else:
