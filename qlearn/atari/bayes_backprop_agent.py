...
class AtariBayesBackpropDQN(nn.Module):
...
class AtariBayesBackpropAgent(object):
    def __init__(self, args, input_dim, num_actions):
        ...
        self.kl_coeff = args.alpha / args.replay_buffer_size
        self.online_net = AtariBayesBackpropDQN(args, input_dim, num_actions)
        self.online_net.train()

        self.target_net = AtariBayesBackpropDQN(args, input_dim, num_actions)
    ...
    def learn(self, states, actions, rewards, next_states, terminals):
        ...
        td_errors = F.smooth_l1_loss(state_action_values, target_state_action_values.detach())
        # loss = td_errors
        kl_reg = self.online_net.get_reg()
        loss = td_errors + self.kl_coeff * kl_reg
        # Optimize the model
