...
from qlearn.atari.local_mnf_agent import AtariMNFAgent
...
def parse_args():
    parser = argparse.ArgumentParser("MNF DQN experiments for Atari games")
    # Environment
    ...
    parser.add_argument("--agent", type=str, default="MNFDQN", help="which agent to run")
    ...
    parser.add_argument('--n-flows-q', type=int, default=2, help='number of normalizing flows using for the approximate posterior q')
    parser.add_argument('--n-flows-r', type=int, default=2, help='number of normalizing flows using for auxiliary posterior r')
    parser.add_argument('--alpha', type=float, default=1.0, help='trade-off parameter betweem KL and likelihood term')
...
if __name__ == '__main__':
    ...
    agent = AtariMNFAgent(args, env.observation_space.shape[-1], num_actions)
    ...
    while True:
        ...
        if done:
            ...
            #     monitor = csv.writer(monitor_file)
            #     monitor.writerow([num_iters, episode_rewards[-1]])
